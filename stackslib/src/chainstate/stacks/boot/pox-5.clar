...
;; Default length of the period between unlocking and start of cycle, in burnchain blocks.
(define-constant UNLOCKED_PERIOD_LENGTH (if is-in-mainnet u100 u50))
...
(define-data-var pox-unlocked-period-length uint UNLOCKED_PERIOD_LENGTH)
...
(define-public (set-burnchain-parameters (first-burn-height uint)
    ...
    (begin
        ...
        (var-set first-pox-5-reward-cycle begin-pox-5-reward-cycle)
...
;;  and the `pox-5` contract tries to keep this state in sync with the reward-cycle
...
;;    (6) (reward-cycle-to-unlock-burn-height (+ lock-period first-reward-cycle)) == (get unlock-height (stx-account stacker))
...
(define-read-only (reward-cycle-to-unlock-burn-height (cycle uint))
    (- (+ (var-get first-burnchain-block-height) (* cycle (var-get pox-reward-cycle-length))) (var-get pox-unlocked-period-length)))
    ...
    (let ((first-reward-cycle (+ u1 (current-pox-reward-cycle)))
      ...
      (let ((reward-set-indexes (try! (add-pox-addr-to-reward-cycles pox-addr first-reward-cycle lock-period amount-ustx tx-sender signer-key))))
         ...
         (map-set stacking-state
          ...
          (ok { stacker: tx-sender, lock-amount: amount-ustx, signer-key: signer-key, unlock-burn-height: (reward-cycle-to-unlock-burn-height (+ first-reward-cycle lock-period)) }))))
...
;; The domain is `{ name: "pox-5-signer", version: "1.0.0", chain-id: chain-id }`.
(define-read-only (get-signer-key-message-hash (pox-addr { version: (buff 1), hashbytes: (buff 32) })
  ...
  (sha256 (concat
    SIP018_MSG_PREFIX
    (concat
      (sha256 (unwrap-panic (to-consensus-buff? { name: "pox-5-signer", version: "1.0.0", chain-id: chain-id })))
      (sha256 (unwrap-panic
        (to-consensus-buff? {
          pox-addr: pox-addr,
          reward-cycle: reward-cycle,
...
(define-public (delegate-stack-stx (stacker principal)
    ...
    (let ((first-reward-cycle (+ u1 (current-pox-reward-cycle)))
          (specified-reward-cycle (+ u1 (burn-height-to-reward-cycle start-burn-ht)))
          (unlock-burn-height (reward-cycle-to-unlock-burn-height (+ (current-pox-reward-cycle) u1 lock-period))))
      ;; the start-burn-ht must result in the next reward cycle, do not allow stackers
      ;;  to "post-date" their `stack-stx` transaction
      (asserts! (is-eq first-reward-cycle specified-reward-cycle)
                (err ERR_INVALID_START_BURN_HEIGHT))
...
(define-public (stack-increase 
  ...
  (auth-id uint))
   (let ((stacker-info (stx-account tx-sender))
      ...
      ;; NOTE: stacking-state map is unchanged: it does not track amount-stacked in PoX-5
...
(define-public (stack-extend (extend-count uint)
   ...
   (let ((stacker-info (stx-account tx-sender))
    ...
    (let ((last-extend-cycle  (- (+ first-extend-cycle extend-count) u1))
          ...
          (new-unlock-ht (reward-cycle-to-unlock-burn-height (+ u1 last-extend-cycle))))
